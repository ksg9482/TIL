...
### flask의 blueprint
한 파일에 모든 기능 코드를 넣으면 복잡해지고 관리가 어려워진다.
* re-usability를 항상 생각해야 한다
  * 단, 대형 프로그램에 필요한 구조를 가져갈 필요는 없다

플라스크는 어플리케이션 컴포넌트를 만들고 어플리케이션 내부나 어플리케이션간에 공통 패턴을 지원하기 위해 블루프린트를 사용한다
블루프린트 객체는 플라스크 어플리케이션 객체와 유사하게 동작하나 실제 어플리케이션은 아니다. 어플리케이션을 생성하거나 확장하는 방식에 대한 블루프린트이다.

* 블루프린트는 어플리케이션에 등록될 수 있는 동작의 집합
* 기본개념은 어플리케이션에 블루프린트가 등록될 때 실행할 동작을 기록한다. 플라스크는 요청을 보내고 하나의 끝점에서 다른 곳으로 URL을 생성할때 뷰 함수와 블루프린트의 연관을 맺는다.

#### 블루프린터에 등록해서 사용
1. 서브폴더 생성하고 코드 작성
2. 블루프린트 객체 생성
3. main에서 블루프린트에 등록된 객체를 호출
